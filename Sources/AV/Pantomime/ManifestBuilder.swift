            ...
            if line.isEmpty {
                ...
                if line.hasPrefix("#EXTM3U") {
                    ...
                    do {
                        ...
                        let widthString = try line.replace("(.*)RESOLUTION=(\\d+)x(\\d+),(.*)", replacement: "$2")
                        let heightString = try line.replace("(.*)RESOLUTION=(\\d+)x(\\d+),(.*)", replacement: "$3")
                        if let currentMediaPlaylistExist = currentMediaPlaylist {
                            if let programId = Int(programIdString) {
                                currentMediaPlaylistExist.programId = programId
                            }
                            if let bandwidth = Double(bandwidthString) {
                                currentMediaPlaylistExist.bandwidth = bandwidth
                            }
                            if let width = Int(widthString) {
                                currentMediaPlaylistExist.width = width
                            }
                            if let height = Int(heightString) {
                                currentMediaPlaylistExist.height = height
                            }
                        }
                    } catch {
